...
export default function NotFound() {
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    if (searchQuery.trim()) {
      router.push(`/${locale}/help?q=${encodeURIComponent(searchQuery.trim())}`);
    }
  };
        ...
        <div className="mb-16 max-w-lg mx-auto relative z-10">
          <form onSubmit={handleSearch} className="relative">
            <label htmlFor="search-404" className="sr-only">
              {locale === 'fi' ? 'Hae apukeskustasta' : 'Search our help center'}
            </label>
            <input
              id="search-404"
              type="text"
              ...
              placeholder={locale === 'fi' ? 'Hae apukeskustasta...' : 'Search our help center...'}
              ...
              aria-label={locale === 'fi' ? 'Hae apukeskustasta' : 'Search our help center'}
            />
            <button
              type="submit"
              className="absolute right-2 top-1/2 -translate-y-1/2 p-2 text-forest hover:text-turquoise transition-colors"
              aria-label={locale === 'fi' ? 'Lähetä haku' : 'Submit search'}
            >
