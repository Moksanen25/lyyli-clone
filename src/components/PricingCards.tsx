...
const plans: PricingPlan[] = [
  ...
  },
  {
    ...
    description:
      'Advanced features for content optimization, campaigns and reporting',
    features: [
  ...
  ({ fullWidth = false, locale = 'en', translations }: PricingCardsProps) => {
    ...
    const translateFeature = (feature: string): string => {
      const featureMap: { [key: string]: string } = {
        'Guided AI onboarding in less than 5 minutes':
          'pricing.features.guidedOnboarding5Min',
        'Proactive communication ideas': 'pricing.features.proactiveIdeas',
        'Broad tone of voice and brand settings for internal and external communications':
          'pricing.features.broadToneBrand',
        ...
        'AI image generation with brand references':
          'pricing.features.aiImageGeneration',
        'All available integrations':
          'pricing.features.allAvailableIntegrations',
        'Priority support': 'pricing.features.prioritySupport',
        'Everything in Launch': 'pricing.features.everythingInLaunch',
        'Up to 1000 messages a month':
          'pricing.features.upToThousandMessagesPerMonth',
        'Priority support (email)': 'pricing.features.prioritySupportEmail',
        'Everything in Growth': 'pricing.features.everythingInGrowth',
        'Advanced AI-analytics to optimize your content and publishing':
          'pricing.features.advancedAIAnalytics',
        'AI-enhanced Campaign mode to create and manage communication campaigns':
          'pricing.features.campaignMode',
        'Editor mode': 'pricing.features.editorMode',
        'AI-enhanced media library with intelligent media suggestions':
          'pricing.features.mediaLibrary',
        'C-level and management automated KPI-reports':
          'pricing.features.kpiReports',
        'Branded templates for different content: communications plan, management report, weekly report, personnel info':
          'pricing.features.brandedTemplates',
        'Priority support (email and phone)':
          'pricing.features.prioritySupportEmailPhone',
        "Local entity in customer's own environment":
          'pricing.features.localEntity',
      ...
      return translationKey
        ? translations?.[translationKey as keyof typeof translations] || feature
        : feature;
    };
    ...
    const getTranslatedPlans = () => {
      return plans.map(plan => ({
        ...
        description:
          translations?.[
            `pricing.${plan.name.toLowerCase()}.description` as keyof typeof translations
          ] || plan.description,
        cta:
          plan.name === 'Professional' || plan.name === 'Enterprise'
            ? translations?.[
                'waitlist.joinWaitlist' as keyof typeof translations
              ] || 'Join waitlist'
            : translations?.[
                `pricing.${plan.name.toLowerCase()}.cta` as keyof typeof translations
              ] || plan.cta,
    ...
    const getPricePeriod = (plan: PricingPlan) => {
      ...
      return billingPeriod === 'monthly'
        ? translations?.['pricing.perMonth'] || '/month'
        : translations?.['pricing.perYear'] || '/year';
    };
      ...
      <section className={`${fullWidth ? 'py-0' : 'py-16 lg:py-24'}`}>
        <div className={fullWidth ? 'w-full px-4' : 'container mx-auto px-4'}>
          ...
          <div className="flex justify-center items-center mb-12">
            ...
            <div
              className={`ml-4 px-3 py-1 text-sm rounded-full transition-colors duration-200 ${
                billingPeriod === 'yearly'
                  ? 'bg-forest text-white'
          ...
          <div className="w-full text-center mt-12">
            <span className="text-sm text-mediumGray font-sans">
              {(
                ...
                'All prices shown are per user per month, billed {period}.'
              ).replace(
                '{period}',
                translations?.[
                  billingPeriod === 'monthly'
          ...
          >
            <div className="bg-gradient-to-br from-white to-turquoise/5 rounded-2xl p-8 shadow-xl border border-turquoise/20 max-w-4xl mx-auto">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                ...
                >
                  {translations?.[
                    'pricing.moreInfo.viewPricing' as keyof typeof translations
                ...
                >
                  {translations?.[
                    'pricing.moreInfo.contactSales' as keyof typeof translations
                  ] || 'Contact sales team'}
                  <svg
                    className="w-5 h-5 ml-2"
                    fill="none"
