...
export default function TeamContacts({
  ...
  return (
    <div>
      <AnimatedCard delayMs={0} className="text-center mb-16">
        <h2 className="text-3xl md:text-4xl mb-6 text-forest font-playfair font-normal leading-snug">
          {t["contact.team.title"]}
        </h2>
        <p className="text-lg text-mediumGray font-sans leading-relaxed max-w-3xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8">
        {contacts.map((contact, index) => (
          <AnimatedCard key={index} delayMs={index * 100}>
            <InteractiveCard className="bg-white/95 backdrop-blur-sm rounded-2xl shadow-soft hover:shadow-medium transition-all duration-300 ease-out transform hover:-translate-y-1">
              <div className="text-center mb-6">
                <div className={`w-20 h-20 bg-gradient-to-br ${contact.gradient} rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-soft transform transition-transform duration-300 ease-out group-hover:scale-110`}>
                  {contact.icon}
                </div>
                {contact.name ? (
                  <>
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-3 p-3 bg-grayLight/50 rounded-lg hover:bg-grayLight/70 transition-colors">
                  ...
                  <a
                    ...
                    className="text-base text-forest hover:text-turquoise transition-colors duration-200 ease-out font-sans leading-relaxed font-medium"
                  >
                    {contact.email}
                  </a>
                </div>
                ...
                {contact.phone && (
                  <div className="space-y-3">
                    <div className="flex items-center gap-3 p-3 bg-grayLight/50 rounded-lg hover:bg-grayLight/70 transition-colors duration-200 ease-out">
                      <svg
                        className="w-5 h-5 text-forest flex-shrink-0"
                        fill="none"
                        stroke="currentColor"
                      ...
                      <div>
                        <a
                          ...
                          className="text-base text-forest hover:text-turquoise transition-colors duration-200 ease-out block font-sans leading-relaxed font-medium"
                        >
                          {contact.phone}
                        </a>
                    ...
                    {contact.name === t["contact.team.mikko.name"] && (
                      <div className="flex gap-2">
                        ...
                        <a
                          href={`tel:${contact.phone}`}
                          className="flex-1 bg-forest hover:bg-forest/80 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors duration-200 ease-out flex items-center justify-center gap-2"
                        >
