      ...
      >
        <defs>
          ...
          >
            ...
            <stop offset="50%" stopColor="#A7D6D1" />
            <stop offset="100%" stopColor="#F7EBEB" />
          </linearGradient>
          <radialGradient id="radialGradient" cx="50%" cy="50%" r="50%">
            <stop offset="0%" stopColor="#F7EBEB" stopOpacity="0.1" />
        ...
        <circle
          ...
          r="8"
          fill="currentColor"
          className="text-turquoise opacity-20 transition-all duration-3000"
        />
        <circle
          cx="80"
          cy="70"
          r="12"
          fill="currentColor"
          className="text-rose opacity-20 transition-all duration-2500"
        />
        <rect
          x="60"
          y="10"
          ...
          height="15"
          fill="currentColor"
          className="text-forest opacity-10 transition-all duration-4000"
        />
        ...
        <polygon
          ...
          fill="currentColor"
          className="text-turquoise opacity-20 transition-all duration-3500"
        />
...
export function FloatingElements() {
  ...
  return (
    <div ref={containerRef} className={baseClasses}>
      ...
      <div
        className={`absolute w-4 h-4 rounded-full bg-turquoise opacity-25 transition-all duration-1000 ${
      ...
      />
      <div
        className={`absolute w-6 h-6 rounded-full bg-rose opacity-20 transition-all duration-1000 ${
      ...
      />
      <div
        className={`absolute w-3 h-3 rounded-full bg-forest opacity-30 transition-all duration-1000 ${
      ...
      <div
        className={`absolute w-2 h-2 rounded-full bg-turquoise opacity-40 transition-all duration-1000 ${
          isClient ? "animate-pulse" : "opacity-0"
        }`}
        style={{
          left: "40%",
          top: "80%",
          transform: isClient
            ? `translate(${-mousePosition.x * 0.008}px, ${-mousePosition.y * 0.008}px)`
            : "translate(0px, 0px)",
          animationDelay: "600ms",
        }}
      />
      <div
        className={`absolute w-5 h-5 rounded-full bg-rose opacity-25 transition-all duration-1000 ${
          isClient ? "animate-bounce" : "opacity-0"
        }`}
        style={{
          left: "90%",
          top: "15%",
          transform: isClient
            ? `translate(${mousePosition.x * 0.012}px, ${mousePosition.y * 0.012}px)`
            : "translate(0px, 0px)",
      ...
      <div
        className={`absolute w-4 h-4 bg-forest opacity-20 transition-all duration-1000 ${
        ...
        style={{
          ...
          top: "50%",
          transform: isClient
            ? `translate(${mousePosition.x * 0.005}px, ${mousePosition.y * 0.005}px) rotate(45deg)`
            : "translate(0px, 0px) rotate(45deg)",
          animationDelay: "1000ms",
        }}
      />
      <div
        className={`absolute w-3 h-3 bg-turquoise opacity-35 transition-all duration-1000 ${
          isClient ? "animate-pulse" : "opacity-0"
        }`}
        style={{
          left: "70%",
          top: "85%",
          transform: isClient
            ? `translate(${-mousePosition.x * 0.018}px, ${-mousePosition.y * 0.018}px)`
            : "translate(0px, 0px)",
...
export function AICommunicationVisual() {
  ...
  return (
    <div ref={ref} className="relative w-full h-64">
      ...
      >
        <div
          className={`w-16 h-16 bg-gradient-to-br from-forest to-turquoise rounded-full flex items-center justify-center shadow-lg transition-all duration-500 ${
            isAnimating
              ? "scale-110 shadow-xl ring-4 ring-turquoise ring-opacity-30"
              : "scale-100"
          }`}
        >
          <svg
        ...
        <div
          className={`absolute inset-0 w-16 h-16 border-2 border-turquoise rounded-full transition-all duration-1000 ${
            isAnimating ? "scale-150 opacity-0" : "scale-100 opacity-100"
          }`}
        />
        <div
          className={`absolute inset-0 w-16 h-16 border border-rose rounded-full transition-all duration-1500 ${
      ...
      {[
        { angle: 0, color: "turquoise", label: "Team", delay: 0 },
        { angle: 72, color: "rose", label: "Clients", delay: 200 },
        { angle: 144, color: "forest", label: "Partners", delay: 400 },
        {
          angle: 216,
          color: "turquoise",
          label: "Stakeholders",
        ...
        { angle: 288, color: "rose", label: "Vendors", delay: 800 },
      ].map((node, index) => (
        <div
          key={index}
        ...
        >
          <div
            className={`w-8 h-8 bg-${node.color} rounded-full flex items-center justify-center shadow-md transition-all duration-300 hover:scale-110 hover:shadow-lg`}
          >
            <div className="w-3 h-3 bg-white rounded-full" />
          </div>
          <div className="absolute top-full left-1/2 transform -translate-x-1/2 mt-2">
            <span className="text-xs text-mediumGray whitespace-nowrap font-medium">
              {node.label}
            </span>
          </div>
        </div>
        ...
        {[0, 72, 144, 216, 288].map((angle, index) => {
          ...
          return (
            <line
              ...
              y2={y2}
              stroke="currentColor"
              className="text-turquoise opacity-40 transition-all duration-1000"
              strokeWidth="1.5"
...
export function GradientBackground({ className = "" }: { className?: string }) {
  ...
  return (
    <div ref={ref} className={`absolute inset-0 ${className}`}>
      ...
      <div
        className={`absolute inset-0 bg-gradient-to-br from-rose via-white to-turquoise opacity-40 transition-all duration-2000 ${
          isVisible ? "opacity-40" : "opacity-0"
        }`}
      />
      <div
        className={`absolute inset-0 bg-gradient-to-tl from-forest via-transparent to-rose opacity-15 transition-all duration-2000 delay-300 ${
      ...
      <div
        className={`absolute inset-0 bg-gradient-to-r from-turquoise via-transparent to-rose opacity-10 transition-all duration-2000 delay-600 ${
          isVisible ? "opacity-10" : "opacity-0"
        }`}
      />
      <div
        className={`absolute inset-0 bg-gradient-to-b from-transparent via-forest to-transparent opacity-5 transition-all duration-2000 delay-900 ${
      ...
      <div
        className={`absolute inset-0 bg-gradient-radial from-rose via-transparent to-transparent opacity-20 transition-all duration-2000 delay-1200 ${
      ...
      <div
        className={`absolute inset-0 bg-gradient-to-r from-transparent via-turquoise to-transparent opacity-5 transition-all duration-10000 ${
    ...
    >
      ...
      >
        ...
        <path
          ...
          opacity="0.3"
          fill="currentColor"
          className="text-forest transition-all duration-3000"
        />
        <path
          ...
          opacity="0.6"
          fill="currentColor"
          className="text-turquoise transition-all duration-3000 delay-200"
        />
        <path
          d="M0,0V5.63C149.93,59,314.09,71.32,475.83,42.57c43-7.64,84.23-20.12,127.61-26.46,59-8.63,112.48,12.24,165.56,35.4C827.93,77.22,886,95.24,951.2,90c86.53-7,172.46-45.71,248.8-84.81V0Z"
          fill="currentColor"
          className="text-rose opacity-80 transition-all duration-3000 delay-400"
        />
      </svg>
    </div>
  );
...
export function ProgressBar({
  ...
  return (
    ...
    >
      ...
      <div className="w-full bg-grayLight rounded-full h-2 overflow-hidden">
        <div
...
export function AnimatedCounter({
  ...
  return (
    <div
      ref={ref}
      className={`text-4xl font-bold text-forest transition-all duration-1000 ${
...
export function FeatureHighlightCard({
  ...
  return (
    <div
      ref={ref}
      className={`group relative bg-white rounded-2xl shadow-soft p-6 transition-all duration-1000 hover:shadow-xl hover:-translate-y-2 ${
        isClient && isVisible
    ...
    >
      ...
      <div
        className={`absolute inset-0 bg-gradient-to-br ${gradient} rounded-2xl opacity-0 transition-opacity duration-500 group-hover:opacity-5`}
      />

      {/* Icon container with enhanced styling */}
      <div
        className={`relative z-10 w-16 h-16 bg-gradient-to-br ${gradient} rounded-2xl flex items-center justify-center mb-6 transition-all duration-500 group-hover:scale-110 group-hover:shadow-lg ${
      ...
      {/* Subtle border effect with brand colors */}
      <div className="absolute inset-0 rounded-2xl border border-transparent transition-all duration-500 group-hover:border-turquoise group-hover:border-opacity-20" />
    </div>
  );
}
...
export function AnimatedTimeline({
  ...
  return (
    <div className={`relative ${className}`}>
      {/* Timeline line */}
      <div className="absolute left-8 top-0 bottom-0 w-0.5 bg-gradient-to-b from-turquoise to-rose transition-all duration-1000 opacity-100 scale-y-100" />
        ...
        {data.map((item, index) => (
          <div key={index} className="space-y-2">
            <div className="flex justify-between items-center">
              ...
              <span className="text-sm text-mediumGray">{item.value}</span>
            </div>
            <div className="w-full bg-grayLight rounded-full h-3 overflow-hidden">
              <div
                className={`h-full bg-${item.color || "turquoise"} rounded-full transition-all duration-2000 ease-out ${
                  isVisible ? "animate-pulse" : ""
                }`}
                style={{
...
export function TestimonialCard({
  ...
  return (
    ...
    >
      {/* Quote icon */}
      <div className="absolute top-4 right-4 text-rose text-3xl opacity-60">
        "
      </div>
      ...
      <div className="flex items-center mb-4">
        {[...Array(5)].map((_, i) => (
          <svg
            key={i}
            className={`w-5 h-5 ${i < rating ? "text-yellow-400" : "text-grayLight"}`}
            fill="currentColor"
            viewBox="0 0 20 20"
          >
      ...
      {/* Quote text */}
      <blockquote className="text-mediumGray mb-6 italic leading-relaxed font-sans">
        "{quote}"
      </blockquote>
...
export function VisualAnimatedBackgroundPattern({
  className = "",
}: {
  className?: string;
}) {
  ...
  return (
    <div ref={ref} className={`absolute inset-0 overflow-hidden ${className}`}>
      ...
      >
        ...
        <div
          className={`absolute top-1/4 left-1/4 w-32 h-32 border border-turquoise opacity-10 rounded-full transition-all duration-8000 ${
            isVisible ? "animate-spin" : ""
          }`}
        />
        <div
          className={`absolute top-3/4 right-1/4 w-24 h-24 bg-rose opacity-5 rounded-full transition-all duration-6000 ${
            isVisible ? "animate-pulse" : ""
          }`}
        />
        <div
          className={`absolute bottom-1/4 left-1/3 w-16 h-16 border border-forest opacity-8 rounded-full transition-all duration-10000 ${
            isVisible ? "animate-spin" : ""
          }`}
