...
export default function ComparisonTable({ translations }: ComparisonTableProps) {
  ...
  const renderCell = (value: string | boolean) => {
    if (typeof value === 'boolean') {
      return value ? (
        <svg className="w-5 h-5 text-forest-green mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
        </svg>
      ) : (
        <svg className="w-5 h-5 text-medium-gray mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
            ...
            <table className="w-full min-w-[800px]" role="table">
              ...
              <tbody className="divide-y divide-light-gray">
                {comparisonData.map((row, index) => (
                  <tr key={index} className={`${index % 2 === 0 ? 'bg-white' : 'bg-soft-rose/30'} hover:bg-soft-rose/50 transition-colors`}>
                    <td className="px-6 py-4 font-medium text-dark-gray">
                      {row.feature}
                    </td>
                    <td className="px-6 py-4 text-center text-medium-gray">
                      {renderCell(row.free)}
                    </td>
                    <td className="px-6 py-4 text-center text-medium-gray">
                      {renderCell(row.starter)}
                    </td>
                    <td className="px-6 py-4 text-center text-medium-gray bg-forest-green/5">
