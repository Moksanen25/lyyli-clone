...
export default function PricingTiers({ translations }: PricingTiersProps) {
  ...
  const renderIllustration = (type: string) => {
    const iconClass = "w-12 h-12 text-forest";
        ...
        return (
          ...
          >
            <path
              ...
              strokeWidth={1.5}
              d="M12 3v17.25m0 0c-1.472 0-2.882.265-4.185.75L12 18.75l4.185 2.25c-1.303-.485-2.713-.75-4.185-.75zM12 12.75c1.472 0 2.882.265 4.185.75L12 15.75l-4.185-2.25c1.303.485 2.713.75 4.185.75z"
            />
          </svg>
        );
      case "rocket":
  ...
  return (
    <div className="space-y-12">
      ...
      <div className="flex justify-center">
        <div className="bg-grayLight rounded-lg p-1 flex items-center">
          <button
        ...
        {tiers.map((tier, index) => (
          <article
            ...
            className={`relative bg-white rounded-lg border-2 p-6 transition-all ${
              tier.popular
                ? "border-forest shadow-xl scale-105 lg:col-span-1 xl:col-span-1 bg-gradient-to-br from-white to-rose/5"
                : "border-grayLight hover:border-turquoise hover:shadow-soft"
            } ${index >= 3 ? "md:col-span-2 lg:col-span-3 xl:col-span-1" : ""}`}
          >
            {/* Glow effect for Professional tier */}
            {tier.popular && (
              <div className="absolute inset-0 bg-gradient-to-br from-forest/10 to-turquoise/10 rounded-lg blur-xl -z-10 opacity-50"></div>
            )}
            
            {tier.popular && (
              <div className="absolute -top-5 left-1/2 transform -translate-x-1/2">
                <span className="bg-gradient-to-r from-forest to-turquoise text-white px-4 py-2 rounded-full text-sm font-semibold whitespace-nowrap shadow-lg border-2 border-white">
                  Professional Choice
                </span>
              </div>
            )}
            ...
            <ul className="space-y-3 mb-8">
              {tier.features.map((feature, featureIndex) => (
                <li key={featureIndex} className="flex items-start gap-3">
                  ...
                  <span className="text-xs text-mediumGray font-sans leading-relaxed">
                    {feature}
                  </span>
                </li>
              ))}
