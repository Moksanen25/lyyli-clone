...
const REQUIRED_HEADERS = {
  'Content-Security-Policy': {
    ...
    validate: (value) => {
      ...
      if (!value.includes("frame-ancestors 'none'")) {
        return { valid: false, reason: 'Missing frame-ancestors \'none\' directive' };
      }
  ...
  'X-Content-Type-Options': {
    ...
    validate: (value) => {
      if (!value) return { valid: false, reason: 'Missing X-Content-Type-Options header' };
      if (value !== 'nosniff') return { valid: false, reason: 'X-Content-Type-Options should be nosniff' };
      return { valid: true };
    }
  },
  'Referrer-Policy': {
    ...
    validate: (value) => {
      if (!value) return { valid: false, reason: 'Missing Referrer-Policy header' };
  ...
  'Permissions-Policy': {
    ...
    validate: (value) => {
      if (!value) return { valid: false, reason: 'Missing Permissions-Policy header' };
      ...
      for (const permission of dangerousPermissions) {
        if (!value.includes(`${permission}=()`)) {
          return { valid: false, reason: `Dangerous permission not blocked: ${permission}` };
        }
      }
  ...
  'X-Frame-Options': {
    ...
    validate: (value) => {
      if (!value) return { valid: false, reason: 'Missing X-Frame-Options header' };
      if (value !== 'DENY') return { valid: false, reason: 'X-Frame-Options should be DENY' };
      return { valid: true };
    }
  },
  'X-XSS-Protection': {
    ...
    validate: (value) => {
      if (!value) return { valid: false, reason: 'Missing X-XSS-Protection header' };
      if (value !== '1; mode=block') return { valid: false, reason: 'X-XSS-Protection should be 1; mode=block' };
      return { valid: true };
    }
  }
};
  ...
  try {
    ...
    for (const [headerName, config] of Object.entries(REQUIRED_HEADERS)) {
      ...
      if (config.required && !headerValue) {
        results.errors.push(`Missing required header: ${headerName} - ${config.description}`);
    ...
    if (headers['x-powered-by']) {
      results.warnings.push('X-Powered-By header exposes framework information');
    }
